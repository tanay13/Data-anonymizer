...
Custom.is_unit = function (date) {
  ...
  for (var i = 0; i < units.length; i++) {
    if (date.indexOf(units[i]) != -1) {
      ...
      if (
        !isNaN(date.substring(0, unit)) &&
        units[i].length + date.substring(0, unit).length == date.length
      ) {
        return true;
      }
    }
...
Custom.capitalise_array_entries = function () {
  ...
  for (var i = 0; i < irreplacables.length; i++) {
    capitalised.push(
      irreplacables[i][0].toUpperCase() +
        irreplacables[i].slice(1).toLowerCase()
    );
  }
...
Custom.not_replacable = function (date) {
  ...
  if (
    Util.ident_inArray(date, irreplacables) ||
    Util.ident_inArray(date, upper_irreplacables)
  ) {
    return true;
  } else {
    return false;
...
Custom.is_numeric = function (date) {
  return (
    date.split(" ").length == 1 &&
    !isNaN(nlp.value(date).number) &&
    nlp.value(date).number != "" &&
    !Custom.is_weekday(date) &&
    !Custom.is_ordinal(date)
  );
};
...
Custom.has_numeric = function (date) {
  ...
  for (var i = 0; i < split.length; i++) {
    if (
      !isNaN(split[i]) ||
      (!isNaN(nlp.value(split[i]).number) && nlp.value(split[i]).number != "")
    ) {
      return true;
    }
  }
...
Custom.is_weekday = function (date) {
  for (var i = 0; i < weekdays.length; i++) {
    if (
      weekdays[i].toLowerCase() == date.toLowerCase() ||
      (weekdays[i].toLowerCase() ==
        date.substring(0, date.length - 1).toLowerCase() &&
        date.substring(date.length - 1) == "s")
    ) {
      return true;
    }
  }
...
Custom.return_numeric_date = function (date) {
  ...
  if (separator == "none") {
    new_date =
      Custom.get_month() +
      " " +
      Custom.get_number(30) +
      " " +
      Custom.get_year(date, separator);
  } else if (separator == ".") {
    new_date =
      "" +
      Custom.get_number(30) +
      separator +
      Custom.get_number(12) +
      separator +
      Custom.get_year(date, separator);
  } else {
    new_date =
...
Custom.get_year = function (date, separator) {
  ...
  if (diff < 0) {
    return (
      parseInt(year) +
      Math.floor(
        Math.random() * (parseInt(year) - parseInt(present.getFullYear()))
      ) +
      1
    );
  } else {
...
Custom.is_ordinal = function (string) {
  ...
  if (value != null) {
    if (
      (value_comp == 11 || value_comp == 12 || value_comp == 13) &&
      string.substring(string.length - 2) == "th"
    ) {
      return true;
    }
